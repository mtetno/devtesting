...
/**
 * Lay security around Server side code
 * 
 * @author deepak
 */
@Configuration
...
class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		http.sessionManagement().sessionCreationPolicy(STATELESS).and().exceptionHandling()
				...
				.antMatchers(COMPANY).hasAnyAuthority(ROLE.ADMIN.toString()).and().authorizeRequests()
				.antMatchers("/health").permitAll().and().authorizeRequests().antMatchers("/user/addUser").permitAll()
				.and()
				// .authorizeRequests().antMatchers(APPLICATION).hasAnyAuthority(ROLE.SUPER.toString()).and()
				// .authorizeRequests().requestMatchers(new NegatedRequestMatcher(new
				// OrRequestMatcher(new
				// AntPathRequestMatcher(COMPANY)))).hasAnyAuthority(ROLE.strValues()).and()
