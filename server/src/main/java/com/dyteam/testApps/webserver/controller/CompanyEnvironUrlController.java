...
public class CompanyEnvironUrlController {
    ...
    public Boolean delete(@AuthenticationPrincipal final LoginUser loggedInUser,
        ...
        companyEnvironUrlRepo.deleteById(environmentUrlId);
        return true;
    }
    ...
    public AppCompanyEnvWrapper findAllByCompanyId(@AuthenticationPrincipal final LoginUser loggedInUser) {
        logger.info("get all companyEnvironUrls");
        Iterable<ApplicationCompanyUrl> companyEnvironUrls = companyEnvironUrlRepo
                .findAllByUserId(loggedInUser.getCompanyId());
        // List<Environment> environments =
        // environmentRepo.getAllByCompanyId(loggedInUser.getCompanyId());
        // List<Application> applications =
        // applicationRepo.getAllByCompanyId(loggedInUser.getCompanyId());
        ...
        companyEnvironUrls.forEach(ceu -> {
            if (!map.containsKey(ceu.getApplicationId())) {
                ...
                ApplicationEnvUrlInfo applicationEnvUrlInfo = new ApplicationEnvUrlInfo(ceu.getApplicationName(),
                        companyEnvironUrlLists);
    ...
    /**
     * Fetches a complex object of Company env. list mapped to its respective
     * application
     * 
     * @param loggedInUser
    ...
    public AddCompanyEvnUrl getAllByCompanyId(@PathVariable(value = "applicationId") Long applicationId,
            @AuthenticationPrincipal final LoginUser loggedInUser) {
        logger.info("get all companyEnvironUrls");
        Iterable<Object[]> companyEnvironUrls = companyEnvironUrlRepo.findAllByUserId1(loggedInUser.getCompanyId(),
                applicationId);
        logger.info("get all companyEnvironUrls fom db : " + companyEnvironUrls);
