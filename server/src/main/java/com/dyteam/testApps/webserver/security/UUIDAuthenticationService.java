...
final class UUIDAuthenticationService implements IUserAuthenticationService {
	...
	public LoginUser login(final String loginId, final String password) {
		...
		logger.info("passEncoder" + passEncoder.encode("1"));
		...
		if (findByLoginId.isPresent()) {
			...
			Long lng = new Long("0");
			user = new LoginUser(uuid, userEntity.getId(), loginId, password, lng, authorities);
	...
	public UserDetails altlogin(String username, String password) {
		...
		if (findByLoginId.isPresent()) {
			...
			// user = new User(uuid, userEntity.getUserId(),username,
			// password,userEntity.getCompanyId(),authorities);
